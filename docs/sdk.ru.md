...
При подключении Edge передаёт Buywell полный подписанный manifest пакета.
Благодаря этому частный модуль с `preserve-v1` обнаруживается так же, как
официальный: его события, биндинги, резолверы и блоки становятся доступны
только после проверки подписи, digest и точной версии контракта.
